...
The CA is persisted in macOS Keychain and shared across sessions. It regenerates automatically every 24 hours:

- **First run of the day**: Generates a new ECDSA P-256 CA, stores it in Keychain, and adds it to the user trust store (one biometric/password prompt)
- **Subsequent runs**: Loads from Keychain with zero prompts
- **After 24h**: Removes the expired CA and regenerates (one prompt)
